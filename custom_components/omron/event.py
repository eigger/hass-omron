"""Support for omron event entities."""
...
class OmronEventEntity(EventEntity):
    """Representation of a Omron event entity."""
    ...
    def __init__(
        ...
        address: str,
        event_class: str,
        event: OmronBleEvent | None,
    ) -> None:
        """Initialise a Omron event entity."""
    ...
    @callback
    def _async_handle_event(self, event: OmronBleEvent) -> None:
        self._trigger_event(event[EVENT_TYPE], event[EVENT_PROPERTIES])
...
async def async_setup_entry(
    ...
    async_add_entities(
        # Matches logic in PassiveBluetoothProcessorEntity
        OmronEventEntity(address_event_class[0], address_event_class[2], None)
    ...
    @callback
    def _async_discovered_event_class(event_class: str, event: OmronBleEvent) -> None:
        """Handle a newly discovered event class with or without a postfix."""
        async_add_entities([OmronEventEntity(address, event_class, event)])
